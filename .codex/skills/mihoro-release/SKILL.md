---
name: mihoro-release
description: 用于 mihoro-cli 的项目专属版本发布流程。当用户要求发布版本、更新版本号、更新 CHANGELOG、打 annotated tag、运行 pnpm run pack:release、推送 origin，或明确要求执行 mihoro-cli 版本发布技能时使用。
---
...
只在 `mihoro-cli` 使用本技能。发布流程必须更新版本号和 `CHANGELOG.md`，必须把 npm tarball 生成到 `releases/`，必须创建 annotated tag，必须推送当前分支和 tag 到 `origin`。
...
3. `pnpm run build`
4. `pnpm run pack:release`
5. 确认 `releases/mihoro-cli-<version>.tgz` 存在。

禁止运行裸 `pnpm pack`。禁止让 tarball 生成到项目根目录或 `dist/`。
